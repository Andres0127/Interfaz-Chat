...
app.get('/api/amigos/:consecuser', async (req, res) => {
  ...
  try {
    connection = await getConnection();
    // Consulta compleja: Obtener todos los amigos del usuario y el último mensaje intercambiado con cada uno
    // Estructura general:
    //   SELECT ... FROM AMIG_ a JOIN USUARIO u ...
    //   UNION
    //   SELECT ... FROM AMIG_ a JOIN USUARIO u ...
    //   ORDER BY ULTIMO_MENSAJE DESC NULLS LAST
    // Detalle:
    // - Se hace un UNION de dos consultas para cubrir ambas direcciones de amistad (usuario puede ser A o B en la relación).
    // - Cada SELECT obtiene:
    //   - Datos del amigo (CONSECUSER, NOMBRE, APELLIDO, etc.)
    //   - ULTIMO_MENSAJE: subconsulta que busca la fecha/hora del mensaje más reciente entre el usuario y ese amigo.
    //   - ULTIMO_TEXTO: subconsulta que busca el texto del último mensaje (solo si es de tipo texto, IDTIPOCONTENIDO='2').
    //   - ULTIMO_REMITENTE: subconsulta que obtiene el remitente (CONSECUSER) del último mensaje.
    // - JOIN USUARIO u: une la relación de amistad con los datos del usuario amigo.
    // - WHERE y subconsultas: filtran para obtener solo los mensajes entre el usuario y ese amigo, y solo el más reciente.
    // - ORDER BY ULTIMO_MENSAJE DESC NULLS LAST: ordena los chats por el mensaje más reciente, dejando los que no tienen mensajes al final.
    // Propósito: Mostrar la lista de chats con amigos, mostrando el último mensaje, su remitente y la fecha/hora.
    const result = await connection.execute(
      `SELECT * FROM (
